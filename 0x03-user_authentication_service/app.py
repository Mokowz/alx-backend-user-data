...
def login():
    ...
    Returns request with form data
    """
    user_req = request.form
    user_email = user_req.get('email', '')
    user_pass = user_req.get('password', '')
    valid_log = AUTH.valid_login(user_email, user_pass)
    if not valid_log:
        abort(401)
...
def logout():
    ...
    Find user with requested session ID, if exists
    """
